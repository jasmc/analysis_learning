...
match str(path_home):
	...
	case r'D:\2024 09_Delay 2-P 4 planes JC neurons':
		...
		num_reps_per_plane = 2  # number_reps_plane_consective

		relevant_cs = [range(5,15),
					range(15,25), range(25,55), range(55,45), range(45,55),
					range(55,65), range(65,75), range(75,85)]

		# Build index_list efficiently and with clearer variable names
		index_list = [
			np.concatenate([
...
#* Fix 'AbsoluteTime' so that the time of each frame becomes closer to the time at which the frames were acquired by the data and not when they were caught by the computer.
# The delay between acquiring and catching the frame is unknown and therefore disregarded.
# Use vectorized calculation for linspace start/stop
...
#end#! Need to join with galvo before doing this.
#! #* Interpolate data to the expected framerate.
# data = interpolate_data(data, predicted_framerate)
#endregion
...
##   
#region Galvo signal
galvo = pd.read_csv(galvo_path, sep='\t', decimal=',', usecols=[0,1], names=['AbsoluteTime', 'GalvoValue'], dtype={'GalvoValue':'float64'}, parse_dates=['AbsoluteTime'], date_format=r'%d/%m/%Y  %H:%M:%S,%f', skip_blank_lines=True, skipinitialspace=True, nrows=p.nrows).dropna(axis=0)
...
# 	first_timepoint_galvo = galvo['AbsoluteTime'].iat[0]

# plt.plot(galvo['AbsoluteTime'])
...
galvo = pd.merge_ordered(galvo, pd.DataFrame({'AbsoluteTime': np.arange(beg_image_to_consider_time - number_images_before_first_image_to_consider * interframe_interval, beg_image_to_consider_time, interframe_interval), 'Frame beg': np.ones(number_images_before_first_image_to_consider)}))
...
if (first_timepoint_galvo := galvo['AbsoluteTime'].iat[0]) <= (first_timepoint_data := data['AbsoluteTime'].iat[0]):
	...
	# number_images_discard = len(galvo[(galvo['AbsoluteTime'] < first_timepoint_data) & (galvo['Frame beg'].notna())])
...
data.rename(columns={'AbsoluteTime' : 'Time (ms)'}, inplace=True)
					...
					# data.rename(columns={'AbsoluteTime' : 'Time (ms)'}, inplace=True)
					# data['Time (ms)'] -= data['Time (ms)'].iat[0]
...
# first_timepoint = galvo['Time (ms)'].iat[0]

# galvo['Time (ms)'] -= first_timepoint


# data['Time (ms)'] -= first_timepoint
# data = data[data['Time (ms)'] >= 0]

# galvo = galvo[galvo['Time (ms)'] <= data['Time (ms)'].iat[-1]]

# fig, axs = plt.subplots(2, 1, figsize=(10, 8))
# axs[0].plot(galvo['Time (ms)'])
# axs[0].set_title('Galvo Signal')
# axs[1].plot(data['Time (ms)'])
# axs[1].set_title('Data')
# plt.tight_layout()
# plt.show()

# plt.plot(data['Time (ms)'].to_numpy())
# plt.plot(galvo['Time (ms)'].to_numpy())

# data['Time (ms)'] -= data['Time (ms)'].iat[0]

# del galvo
# data = pd.merge_ordered(data, galvo, on='Time (ms)', how='outer')
...
for stim_number_i, stim_number in enumerate(stim_numbers):
	...
	# axs[stim_number_i].plot(data_plot['Time (ms)'].to_numpy() - us_beg_, data_plot['Angle (deg) 14'].diff().to_numpy(), 'k.')
	# axs[stim_number_i].plot(data_plot['Time (ms)'].to_numpy() - us_beg_, data_plot[galvo_value].to_numpy() + 100, 'bo')
	#! axs[stim_number_i].plot(data__['Time (ms)'].to_numpy() - us_beg_, data__[galvo_value].to_numpy(), 'm')
	...
	# data__.plot(x='Time (ms)', y=['Frame beg', 'Image mean', us, us_end], ls='.')
...
#! behavior = data[['Time (ms)'] + data_cols].dropna().rename(columns={'Frame number' : 'Frame number (behavior)'}).copy()
...
imaging = xr.DataArray(images, coords={'index': ('Time (ms)', data.loc[data['Frame beg'].notna(), :].index), 'Time (ms)': data.loc[data['Frame beg'].notna(), 'Time (ms)'].to_numpy(), 'x': range(images.shape[1]), 'y': range(images.shape[2])}, dims=['Time (ms)', 'x', 'y'])
...
for i in range(len(all_data)):
	for j in range(len(all_data[0].trials)):
		...
		axs[i,j].set_xticks([])
		axs[i,j].set_yticks([])
		# Optional: Set axes background if needed
