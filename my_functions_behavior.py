...
# number_frames
	...
	data['Original frame number'] = data[frame_id]
	...
	data.loc[:, cols_to_use_orig[1:]] = data.loc[:, cols_to_use_orig[1:]] * (180/np.pi)
...
def tracking_errors(data, single_point_tracking_error_thr):
	...
	if ((a := data.loc[:,cols[1:]].abs().max()) > single_point_tracking_error_thr).any():
		print('Possible tracking error! Max(abs(angle of individual point)):')
		print(a)

		errors = True

	if data.loc[:,cols[1:]].isna().to_numpy().any():
...
def filter_data(data, space_bcf_window, time_bcf_window):
	...
	data_.iloc[:, 2:-1] = np.mean(rolling_window(data_.loc[:,cols[1:]].to_numpy(), space_bcf_window), axis=2)
	...
	# data_.loc[:,cols[1:]] = data_.astype('float').loc[:,cols[1:]].rolling(window=space_bcf_window, center=True, axis=1).mean()

	#* Filter with a rolling average in time
	data_.loc[:,cols[1:]] = data_.loc[:,cols[1:]].rolling(window=time_bcf_window, center=True, axis=0).mean()
