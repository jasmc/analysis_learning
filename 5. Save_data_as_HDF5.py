"""
5. Export Data to HDF5 Format

This script exports processed imaging and behavioral data to HDF5 format for long-term storage,
sharing, and compatibility with external analysis tools.

Intended Workflow:
1. Load processed data from pickle files (output of scripts 3.x or 4.x)
2. Organize data into hierarchical HDF5 structure:
   - /anatomical_stack: reference anatomy across planes
   - /planes/plane_N/trial_M/: individual trial data
     - protocol: stimulus timing information
     - behavior: tail tracking data
     - images: registered imaging frames
     - responses: calculated activity responses (optional)
3. Apply compression (gzip, level 4) to reduce file size
4. Set optimal chunking for efficient data access patterns
5. Add metadata attributes (experiment info, parameters, timestamps)

HDF5 Structure:
```
fish_data.h5
├── anatomical_stack_images [Z, Y, X]
├── planes/
│   ├── plane_0/
│   │   ├── trial_0/
│   │   │   ├── protocol [time, stim_cols]
│   │   │   ├── behavior [time, features]
│   │   │   ├── images [time, Y, X]
│   │   │   ├── template_image [Y, X]
│   │   │   └── responses/ (optional)
│   │   │       ├── cs_us_vs_pre [Y, X]
...
Key Features:
- Compression: gzip level 4 for space efficiency
- Chunking: Optimized for time-series and spatial access patterns
- Compatibility: Standard HDF5 format readable by MATLAB, Python, R
- Metadata: Preserves experimental parameters and processing history

Dependencies:
- h5py: HDF5 interface for Python
- pandas: For structured data export (optional HDFStore)
- xarray: For netCDF4/HDF5 export of imaging arrays

Note: This script is intended to run after scripts 3.x or 4.x to export final results.
For intermediate storage, pickle files are used for faster Python-native serialization.
"""
...
# fish_name = r'20241022_01_delay_2p-10_mitfaminusminus,ca8e1bgcamp6s_5dpf'
	...
	if len(all_data.planes) > 0:
		...
		if hasattr(all_data.planes[0], 'trials'):
			...
			if len(all_data.planes[0].trials) > 0:
				...
				# Print types and shapes of main fields if present
...
# --- Helper functions for HDF5 Saving ---
...
def _save_attrs(h5_item: Union[h5py.Group, h5py.Dataset], attrs: Dict[str, Any]):
	...
	for key, value in attrs.items():
		try:
			if value is None:
				h5_item.attrs[key] = 'None' # Store None as a string
